	...
	public String follow() {
		...
		while(change)
		{
			...
			for(int i = 0 ; i < rulesSplitted.length ; i++)
			{
				...
				{
					System.out.println(rule[j] +" vvvvvvv");
					boolean noB = true ;
					for(int z = 0 ; z < rule[j].length() ; z ++ )
					{
						...
						if(Character.isUpperCase(rule[j].charAt(z)))
						{
							...
							for(int y = 0 ; y < rulesSplitted.length ; y ++ )
							{
								...
								if(rule1[0].equals(String.valueOf(beta.charAt(x))))
								{
									System.out.println(first[y] + "dflnvdvdvdvdvdv");
									for(int t = 0 ; t < first[y].split(",")[1].length(); t++)
									{
										if(!(firstBeta.contains(String.valueOf(first[y].split(",")[1].charAt(t)))) && !(String.valueOf(first[y].split(",")[1].charAt(t)).equals("e")))
										firstBeta += first[y].split(",")[1].charAt(t);
									}
							...
							if(add)
							{
								...
								System.out.println(follow[1]+" checkkkk");
								...
								for(int m = 0 ; m <follow[i].length() ; m ++)
								{
									...
									System.out.println(firstBeta);
									if(!(follow[index].contains(String.valueOf(follow[i].charAt(m)))))
											{
												if(!(String.valueOf(follow[i].charAt(m)).equals("e"))){
												follow[index] = follow[index] + String.valueOf(follow[i].charAt(m));
												change = true ;
												}
											}
								}
								}
								System.out.println(follow[1]+"checkkkk2");
							}
						}
					}
				}
		...
		{
			FinalAnswer+=rulesSplitted[i].split(",")[0]+",";
			String[] sortArr = follow[i].split("");
			Arrays.sort(sortArr);
			String finalValue = "";
			boolean add$ = false ;
			for( int y = 0 ; y < sortArr.length ; y++)
			{
				if(sortArr[y].contains("$"))
				{
					add$ = true ;
				}
				else
